...
## Mission
Design `user_interface` as the presentation layer described in `architecture.md`: a chat-style UI that forwards user intents to the mission coordinator, shows system status, and collects clarifications. The package stays thin (no planning/execution logic) while offering ergonomic human-in-the-loop controls for the Jazzy + BehaviorTree.ROS2 stack.
...
2. **Command Shortcuts** – Slash-style helpers (`/pause`, `/resume`, `/status`) mapped to mission coordinator services/actions.
...
- **Standard ROS Interfaces**
  - Action client to the mission coordinator’s `ExecuteTree` proxy action.
  - Service clients for `/regenerate_tree`, `/list_subtrees`, `/confirm_stop`, etc. (exact names TBD with the mission coordinator team).
  - Subscriptions to diagnostics topics for status banners.

## Implementation Steps
1. **Skeleton Node** ✅
   - `chat_node.py` now exposes an `AsyncIOExecutor`-backed CLI that reads ROS params (action/service/topic names), writes transcripts, and exposes slash commands for mission coordinator interactions without invoking LangChain directly.
2. **Transport Layer**
...
5. **Testing & Tooling**
   ...
   - CLI smoke test (`ros2 run user_interface chat_node -- --demo`) with mock mission coordinator endpoints.

## Open Questions
- Confirm mission coordinator API (topic/action/service names + schemas) to avoid tight coupling.
- Decide whether LangChain streaming comes via mission coordinator relays or direct WebSocket tokens from the UI.
...
- Web UI needs websocket/server-sent updates & mission coordinator wiring—currently polling JSON + demo responses only.

> Next step: formalize the mission coordinator<->UI contract and scaffold the `chat_node.py` entry point inside this package.
